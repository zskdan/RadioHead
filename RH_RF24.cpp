...
#elif 0
#include "RF24configs/radio_config_Si4463-2.h"
#elif 0
#include "RF24configs/radio_config_Si4463-3.h"
#elif 0
#include "RF24configs/radio_config_Si4463-4.h"
#elif 0
#include "RF24configs/radio_config_Si4463-5.h"
#elif 0
#include "RF24configs/radio_config_Si4463-6.h"
#elif 0
#include "RF24configs/radio_config_Si4463-7.h"
#elif 1
#include "RF24configs/radio_config_Si4463-8.h"
#else
#include "RF24configs/radio_config_Si4461-8bytes.h"
#error
#endif
...
bool RH_RF24::init()
{
	...
	return false; // Unknown radio type, or not connected
     Serial.println("ZSK Part info:"); for(int i=0;i<8;i++) { Serial.print(i) ; Serial.print(":"); Serial.println(buf[i],HEX);} 

    if (!command(RH_RF24_CMD_FUNC_INFO, 0, 0, buf, sizeof(buf)))
	return false; // SPI error? Not connected?

     Serial.println("ZSK Func info:"); for(int i=0;i<8;i++) { Serial.print(i) ; Serial.print(":"); Serial.println(buf[i],HEX);} 
...
{
    uint8_t status[8];
    Serial.print("ZSK HERE:"); Serial.println(__LINE__); 
    command(RH_RF24_CMD_GET_INT_STATUS, NULL, 0, status, sizeof(status));
     Serial.println("ZSK intr status:"); for(int i=0;i<8;i++) { Serial.print(i) ; Serial.print(":"); Serial.println(status[i],HEX);} 

    // Decode and handle the interrupt bits we are interested in
    if (status[0] & RH_RF24_INT_STATUS_CHIP_INT_STATUS) {
	Serial.print("ZSK HERE:"); Serial.println(__LINE__); 
	if (status[6] & RH_RF24_INT_STATUS_CMD_ERROR) {
	    uint8_t chipstatus[3];
	    Serial.print("ZSK HERE:"); Serial.println(__LINE__); 
	    command(RH_RF24_CMD_GET_CHIP_STATUS, NULL, 0, status, sizeof(status));
	    Serial.println("ZSK chip status:"); for(int i=0;i<3;i++) { Serial.print(i) ; Serial.print(":"); Serial.println(chipstatus[i],HEX);} 
        }
    }
    if (status[0] & RH_RF24_INT_STATUS_MODEM_INT_STATUS)
    {
	if ((status[4] & RH_RF24_INT_STATUS_INVALID_PREAMBLE) ||
   	 (status[4] & RH_RF24_INT_STATUS_INVALID_SYNC))
	{
	    ...
	    Serial.println("ZSK modem INTR:"); for(int i=0;i<8;i++) { Serial.print(i) ; Serial.print(":"); Serial.println(status[i],HEX);} 
    }
...
bool RH_RF24::send(const uint8_t* data, uint8_t len)
{
    ...
    if (!waitCAD()) {
	Serial.print("ZSK send failed HERE:"); Serial.println(__LINE__); 
...
{
    Serial.print("ZSK HERE:"); Serial.print(__LINE__); 
    Serial.print(" write into fifo:"); Serial.println(len); 
...
void RH_RF24::sendNextFragment()
{
	...
	_txBufSentIndex += len;
	    ...
	    Serial.print("ZSK HERE:"); Serial.print(__LINE__); 
	    Serial.print(" bufLen: "); Serial.print(_bufLen); 
	    Serial.print(" _txBufSentIndex: "); Serial.println(_txBufSentIndex); 
    }
}
...
{
    Serial.println("ZSK calling SetFreq\n"); 
    while(1);
...
void RH_RF24::setModeTx()
{
	...
	uint8_t tx_params[] = { 0x00, 
				(uint8_t)((_idleMode << 4) | RH_RF24_CONDITION_RETRANSMIT_NO | RH_RF24_CONDITION_START_IMMEDIATE), 0 , 0};
	Serial.println("ZSK tx params:"); for(int i=0;i<2;i++) { Serial.print(i) ; Serial.print(":"); Serial.println(tx_params[i],HEX);} 
...
bool RH_RF24::command(uint8_t cmd, const uint8_t* write_buf, uint8_t write_len, uint8_t* read_buf, uint8_t read_len)
{
    ...
    ATOMIC_BLOCK_END;
    if(done == false){ 
	Serial.print("ZSK command Failed HERE:"); Serial.println(__LINE__); 
    }
